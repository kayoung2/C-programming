...
### 2017학년도 2학기 C프로그래밍과 실습 시간에 했던 강의 내용입니다.
***
...
##### 대입 연산자를 기준으로 왼쪽에 있는 피연산자를 lvalue, 오른쪽에 있는 피연산자를 rvalue라고 한다.
...
### Chapter7 배열
    ...
    자료형 배열명[배열 크기];
##### 배열을 선언함과 동시에 초기화할 때 배열의 원소보다 초깃값을 적게 할당하면, 초깃값이 할당되지 않은 배열의 원소에는 0이 저장된다.
##### 배열의 원소보다 초깃값을 많이 할당하면 오류가 발생한다.

##### 2차원 배열을 선언할 때 행의 개수를 생략할 수 있다. 그러나 열의 개수를 생략하면 오류가 발생한다.
##### 문자 배열(이하 문자열)은 char형 문자를 여러 개 모아놓은 문자 집합으로, 문장도 될 수 있고 단어도 될 수 있다.
##### C언어에는 문자열을 위한 자료형이 별도로 없기 때문에 char형 배열을 이용하여 표현한다.
##### '\0'(NULL)을 통해 문자열의 끝이 어디인지 알기 때문에 문자열 끝에 항상 문자 코드 '\0'을 덧붙여야 한다.
##### 따라서 배열을 문자열로 사용하는 경우에는 배열의 크기를 실제 문자의 개수보다 하나 더 많게 선언해야 한다.

##### 변수에 접근하는 방법은 변수명으로 접근하는 것과 변수의 주소 값으로 접근하는 두 가지가 있다.
##### 변수의 주소는 변수가 메모리에서 만들어진 위치를 나타내므로 값이 유일하고, 변수 앞에 &를 붙여 표시한다.
##### 주소를 출력할 때 변환기호 &p를 이용하여 주소 값을 16진수로 표시했다. 이 주소 값이 메모리에서의 변수 위치를 나타낸다.
...
##### 함수명: 함수명은 식별자 작성 규칙에 따라 함수명만 보고도 함수가 어떤 기능을 수행하는지 알 수 있도록 의미 있게 명명한다.
##### 입력: 함수에 어떤 인자가 입력되는지 나타낸다.
##### 반환 값: 함수가 ㅇ비력을 받아 기능을 수행한 후에 반환되는 결과 값이다.
##### 기능: 함수를 통해 사용자가 구현하려는 내용이다.

+ 재귀함수

  + 함수에서 그 함수를 다시 호출하는 것을 재귀호출이라고 한다.
  + 재귀호출은 데이터 N개로 이뤄진 문제가 N-1개의 문제를 해결하면 간단하게 해결될 때 사용하며, 프로그램을 좀 더 간결하고 알기 쉽게 표현할 수 있다. 
  + 재귀함수는 재귀호출을 구현한 함수다.
...
+ 지역변수
  + 실행되는 단위 블록 내에서만 유효하고 선언된 블록의 영역을 벗어나면 효력이 없다.
  
+ 전역변수
  + 모든 블록에서 사용할 수 있으며 다른 파일에서도 사용할 수 있는 변수다.
  + 전역변수는 함수의 블록 외부에서 선언해야 한다.
  
+ 정적변수
  + 프로그램의 실행이 끝날 때까지 메모리에 계속 보존되는 변수로 전역변수와 생명력이 동일하다.
  
+ 레지스터 변수

      register 자료형 변수명;
      
  + 레지스터 변수의 자료형은 int, char, short 및 포인터형만 사용할 수 있다.
  + 레지스터 변수는 CPU의 레지스터에 저장되므로 처리 속도가 일반 변수보다 빠르다.
  + 반복문의 반복 횟수를 결정하는 변수(첨자)를 레지스터 변수로 사용하는 경우가 많다.

#

### Chapter10 포인터

##### 포인터는 C 언어의 가장 대표적인 특징 중 하나로, 일반 변수의 메모리 내 주소 번지를 가진 자료형이다.
##### 포인터를 사용하면 변수명을 통하지 않고도 사용하고자 하는 대상에 직접 접근할 수 있어 프로그램이 간결하고 효율적으로 제어된다.

##### 포인터 자료형으로 사용되는 연산자에는 주소 연산자와 간접 연산자가 있다.
##### 주소 연산자 &는 일반 변수명에 사용하고 해당 변수의 메모리 주소를 가리킨다.
##### 간접 연산자 * 는 포인터 변수명에 사용하고 주소가 저장되어 있는 메모리 위치를 가리킨다.

     자료형 * 변수명;

##### 포인터를 선언할 때 * 연산자가 하나이면 1차원 포인터다.

##### 배열명은 그 자체로 해당 데이터가 있는 시작 주소를 의미하는 포인터 상수다.

+ 증감 연산자 적용
  + p++: 포인터 p가 가리키는 곳의 다음 주소를 의미한다.
  + * p++: 포인터 p가 가리키는 곳의 다음 주소 안에 들어있는 값을 의미한다.
  + a++: 배열명 a는 상수이므로 증감 연산자를 사용할 수 없다. 즉 배열명에 증감 연산자를 사용하는 것은 3++처럼 상수에 증감 연산자를 사용하는 것과 같다.
